...
* **They are now fitted** by CMA-ES together with the force law and the
  dissipation. Held out, open loop, the twin went from **10.850 deg joint RMS /
  nrmse 0.967 / mean correlation +0.756** at the start of the session to
  **5.639 deg / 0.507 / +0.952** on the first fit (section 10).
* **The fitted weights are not weighings.** The first fit's per-body masses are
  not physically plausible, and the recordings pin only one of them (section 8).
  A refit with the masses modelled as the hardware is built is in section 9.
...
* **A physically parameterized refit** (`8cd4c94`; section 9).
...
**REFIT RESULT — pending.** Until it completes, `canarm_mech.json` is the first
fit of sections 6–8.
...
| (c) | new geometry + axis fix + first mechanical fit | **5.639 deg** | **0.507** | **4.852 / 0.427** | **6.033 / 0.547** | **+0.952** | **+0.969 / +0.976 / +0.911** |
...
The mechanical fit alone took the held-out error from 9.691 to 5.639 deg
(−42 %); against the twin the session began with, the reduction is 48 %. Every
joint improved, and segment 3 most (+0.610 → +0.911). The held-out improvement
exceeds the training one (−29 %), which argues against memorised windows,
although the two losses average different amounts of stillness. The largest
tendon command in the held-out rollout was −39.3 N against the 4000 N clip.
...
in about 0.4 s and joint 2 swings +17.84 deg through the twin receiver.
...
with `hw_tests/results/axis_analysis_2026-08-21.json`. On the first fit:

* 24/24 boards move the measured joint with the measured sign;
* median ratio to the arm 1.00 (range 0.48–1.76); segment medians 0.73 / 0.98 /
  1.23; mean absolute error 3.45 deg;
* `0x101` on joint 2: +17.75 deg against +15.73 deg on the arm (1.13x);
* segment-1 proximal boards are too stiff (0.48–0.58x), segment-3 distal boards
  too soft (1.13–1.67x).
...
`deliverable/twin_vs_real_validation.mp4` (gitignored) and
`deliverable/frame_sample.jpg` were re-rendered from the first fit's rollout. The
frame at 35.00 s shows the ProMax drawing in both panels, with a mean joint error
of 4.41 deg and a worst of 13.43 deg at that instant; joints 0, 4 and 8 of the
twin run in phase with the arm through all 90 s. The caption glyphs were drawn
doubled because OpenCV 5.0 advances thick strokes further than thin ones; the
halo is now drawn at one-pixel offsets (`16339ac`). The camera panel is absent
because the bench camera did not record that session.

## 15. What this does not show, and what to measure next

* **A weight.** The recordings pin the gravity torque each joint sees, not how
  it is split among the bodies, and the total only against `damp_b1` and the
  armature, both carried over.
* **Convergence.** The first fit stopped at its generation limit.
