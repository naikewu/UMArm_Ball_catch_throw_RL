...
| `digital_twin/checkpoints/` | **the fitted twin** `twin_params` loads: `canarm_flow.npz` (flow net; held-out 3682 Pa as trained, 3676 Pa under the corrected kinematics), `canarm_mech.json` (the mechanical fit, with bounds, budget and profiles; held-out numbers in the refine report), `canarm_outer.json` (earlier search, fallback only). `canarm_mech.running.json` = a fit in progress, **gitignored** |
...
| `deliverable/` | the held-out deliverable: `deliverable_validation.json` (metrics + correlation), `frame_sample.jpg` (frame at 35 s), `twin_vs_real_validation.mp4` (~41 MB, **gitignored**) |
...
| `hw_tests/report_canarm_twin_refine_2026-09-10.md` | **the refine write-up**: every fit step and what was not fitted, the ProMax geometry, the qpos-as-q defect, the mass fit and what the recordings pin, the held-out table (10.850 -> 9.691 -> 5.639 deg), the SIM adapter and its timing |
