...
| `canarm_control_gui.py` | **the operator GUI**: 24-board pressure controls, mocap/kinematics strip, plate locks, spawned room viewer, and selectable SIM-only dynamic controller with joint/Cartesian target popup. `--sim` selects `SimMaster(physics="process")` built from the fitted twin; `--self-test` opens no port. Dynamic control uses noisy delayed 240 Hz twin mocap, a separate compute process, and ADC-safe antagonistic pressure targets; pressure edits are locked during control. Its plot slows to 1 Hz and the host uses 0.5 ms GIL handoffs until control stops. The bus is requested at 150 Hz; actual timing is reported separately. KNOWN GAP outside dynamic control: inherited manual bars reach 40 psi without pair-sum enforcement on either adapter. |
...
| `digital_twin/sim_mocap.py` | **the twin's mocap**: `SimMocap(arm, alive=)`, a `CanArmSimStream` fed from live twin joints through the real receiver; defaults remain noise-free (round trip 3.9e-16 rad). Optional seeded joint noise and causal latency; GUI requests 240 Hz, 0.10 deg, 8.33 ms. `latest_control_sample()` atomically carries acquisition time, q and the shared camera-rate velocity estimate. Never steps physics; goes stale when its source closes; `fk_residual_m()` checks twin geometry. No markers, so Lock plates stays live-only. `test_sim_mocap.py` offline |
...
| path | what is there |
|---|---|
| `collection/` | the hardware session: `safety.py` (the operator's 30 psi pair envelope, clamped then asserted, also used by the dynamic GUI bridge), `excitation.py` (the designed signals, each phase justified by the parameter it makes identifiable), `recorder.py` (the 150 Hz JSONL schema plus the raw mocap stream), `resample.py` (the stream onto the sync edges, rate-agnostic), `campaign.py` (bus, mocap, camera, watchdog, bus revival, anomaly watch). `campaign.py --dry-run` opens no port and checks the whole plan |
...
- `control/controller_process.py`, `control/gui_targets.py` — SIM-only spawned compute worker, bounded latest-state IPC with crash/stale watchdogs and ADC-safe pressure transactions, plus 12 joint sliders and base-frame tip target/jog popup in `canarm_control_gui.py`; `test_controller_process.py` covers process failure and target safety.
- `hw_tests/gui_controller_sim_test.py` — real Tk/SIM acceptance with serial/CAN/NatNet opens poisoned: controller selection, slider motion, reachable tip jog, override rejection, child/window/mocap shutdown, and measured command/bus timing; results under `control/results/gui_acceptance*.json`.
- `control/controller.py`, `control/koopman.py` — selectable plain pressure PID, fitted inverse-dynamics feedforward PID, and state-inclusive bilinear Koopman MPPI with synchronized pressure inputs, measured-pair projection, a feedforward prior, and a bounded innovation estimate; `test_controller.py` checks safety, inverse-model ordering, tensor/numpy parity, schema gates and reset reproducibility.
- `control/collect.py`, `control/train.py` — 96 complete simulation episodes across eight excitation families (32 simulated minutes), bounded plant perturbations and distal-body force pulses, then RTX 5090 training with whole-episode train/validation/test splits and optional anchored/frozen-encoder warm starts.
- `control/checkpoints/canarm_koopman.pt`, `canarm_koopman.json`, `campaign_summary.json`, `controller_config.json` — frozen learned model, multistep model report with data hashes, measured campaign coverage, and resolved controller defaults plus the separate multisine tuning trials; raw episodes remain local under `control/data/campaign_20260911/`.
- `control/tune_pid_stress.py`, `control/checkpoints/pid_robust_tuning.json`, `pid_stress_confirmation_configs.json` — independent coupled 10–14 degree hold/multisine PID stress study, initial-pose and plant variation, additional speed-boundary probes, and reproducible final confirmation configs; `test_tune_pid_stress.py` checks analytic derivatives and C2 boundaries.
- `control/render_comparison.py` — same-clock three-controller H.264 comparison from saved poses, with source/reference equality guards, decoded-frame verification and static performance figures.
- `control/summarize.py`, `control/valve_probe.py` — paired-seed benchmark aggregation, held-out model plots, and an equal-input precision/rise-time probe of the fitted valve populations.
- `deliverable/dynamic_control/` — dynamic-controller report, final slow/fast Soft comparison videos, checked frames/manifests, benchmark trial JSON/aggregates, training/valve figures, and explicitly retained development failures; raw rollout NPZs are local and ignored.
- `control/README.md` — controller mechanisms, GUI operation, experiment reproduction and the required episode schema for later anchored warm-start adaptation.
- `control/assets/episode_metadata_example.json` — complete real-episode training metadata example with measured actuator ordering, required sensor fields, explicit provisional values, and episode-length/split requirements.
