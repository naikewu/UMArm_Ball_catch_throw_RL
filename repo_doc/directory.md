...
| `digital_twin/` | the twin, mirroring RS485 `UMArm_SIM`. **`CONTRACT.md` is binding** — every module signature is fixed there; `reference/` holds the 2026-09-10 read of the RS485 implementation (read it instead of the worktree); `README.md` = collect-then-fit plan, `data_schema.md` = JSONL collection schema |
| `digital_twin/mjcf_generator.py` | **the twin's geometry**: `generate_xml`/`build_model`, 24 `pam_k` tendon actuators (`pam_k` = board `0x100+k`), seat azimuths derived from `canarm_actuators.MEASURED_JOINT_PAIRS`, `order="yx"` hinges (so `qpos` is a permutation of `q` — use `q_to_qpos`), masses, 1 ms timestep. Tunables are arguments; `test_mjcf.py` 29/29 offline |
...
| `digital_twin/sim_core.py` | **the twin's centre**: `SimArm` (MuJoCo + 24 nodes + one reentrant lock; `advance_to` is grid-quantised, callable from any thread, and bit-identical under racing advancers), `CanNode`/`SevenMmNode`/`TleNode` (the two firmwares, chosen by **variant byte**), `SimNode` (the plant seam — overrides only `true_psi`, `leak_pa_s`, `_integrate`). Node grid **4 ms** = 3 TLE PID steps (750 Hz) and 4 seven-mm steps (1000 Hz); `sub_step_counts` refuses any other. `placeholder_xml`/`placeholder_actuator` are bring-up stand-ins never reached implicitly. `test_sim_core.py` 34/34 offline |
| `digital_twin/sim_master.py` | **the twin's transport shell**: `SimMaster` subclasses `TLE_PCB/tlelib/backend.py::Backend` and replaces only `self.link` with `SimCanLink`, so the 150 Hz cycle, `set_cycle_observer`, `snapshot_nodes` and `history` are the metal's own code. Per-id reply latency 1.87 ms at `0x101` -> 3.35 ms at `0x118` (64.3 us/id, CAN arbitration). `port` defaults to `"SIM"`; nothing here opens a serial port. `test_sim_master.py` 19/19 offline |
...
| `digital_twin/dataset.py` | **the fitting side's reader**, pure numpy and it must never import torch: `session_*/` -> `Recording` (ADC converted with each board's **own** `NodeCal` from the recorded variant byte), `TendonKinematics` (`q` -> anchored `l`/`ldot` through the twin's MJCF; `MomentArmTendonGeometry` is the labelled fallback while a generator is missing), `make_windows` (multiple-shooting windows re-anchored at their own `p0`, never straddling an episode), `split_episodes` (**whole-episode** train/holdout -- a per-sample split at 150 Hz reports a fantasy holdout), `closed_segments`/`fit_leaks` (closed holds inferred from the commanded error, since this bus has no valve-state bit), and `write_synthetic_session` (a known plant written in the real schema, so one reader serves both). `test_dataset.py` 25/25 offline |
...
| `data/session_*/` | the recordings. **Gitignored**: 273 867 cycles is 300 MB of JSONL |
...
| `digital_twin/deliverable.py` | score the held-out sequence and render the side-by-side video from the SAME rollout |
| `digital_twin/side_by_side.py` | the three-panel compositor: bench camera, model at the measured `q`, model at the predicted `q` |
