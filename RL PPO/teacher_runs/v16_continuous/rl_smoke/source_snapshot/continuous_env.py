...
class ContinuousRelease:
    ...
    def __call__(self, plant, obs):
        ...
        return self.original(plant, obs)
...
class ContinuousEnv(ImprovedTeacherEnv):
    ...
    def residual_mask(self):
        ...
        age = self.obs["t_win"]-self.capture_time
        # After capture, freeze match/gain/blend; only pressure recovery remains active.
        return np.array([0., float(age < .5), 0., 0.], dtype=np.float32)
    ...
    def _set_residual(self, action):
        ...
        if self.capture_time is not None and not mask[1]:
