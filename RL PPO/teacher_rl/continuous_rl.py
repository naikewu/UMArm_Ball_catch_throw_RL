...
def source_hash():
    ...
    for name in ("continuous_env.py", "continuous_rl.py", "continuous_model.py", "continuous_release.py"):
...
def pilot(args):
    ...
    if args.release_audit:
        variants = {"v15_bc":None,
            "ball_fit24":ContinuousConfig(release_ball_frames=24,align_release_clock=True,release_azimuth_bias_deg=0.),
            "ball_motion24":ContinuousConfig(release_ball_frames=24,predict_ball_motion=True,
                align_release_clock=True,release_azimuth_bias_deg=0.),
            "ball_motion32":ContinuousConfig(release_ball_frames=32,predict_ball_motion=True,
                align_release_clock=True,release_azimuth_bias_deg=0.)}
    identity = dict(source_hash=source_hash(), anchor_sha256=file_hash(args.init), manifest=manifest,
        variants={name:None if config is None else asdict(config) for name,config in variants.items()})
...
def main():
    ...
    command.add_argument("--release-audit",action="store_true")
