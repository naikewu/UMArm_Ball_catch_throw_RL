param(
    ...
    [string]$OutputDir = 'teacher_runs/v21_calibrated_teacher/screen_v1',
    [string]$BCCheckpoint = 'teacher_runs/v15_improved/bc_v1_formal/bc_best.pt',
    [string]$Calibration = 'teacher_runs/v21_release_calibration/campaign_v1/release_calibration_stratified.json',
    [int]$Episodes = 0,
    [int]$Workers = 8,
    [int]$Seed = 0
)
...
try {
    ...
    if ($Seed -eq 0) { $Seed = if ($Mode -eq 'Screen') { 22100001 } else { 22200001 } }
    $arguments = @('-u','-m','teacher_rl.calibrated_teacher_rl')
    if ($Mode -eq 'Screen') {
        $arguments += @('screen','--init',$BCCheckpoint,'--calibration',$Calibration,'--out',$OutputDir,
    ...
    & $python @arguments
    if ($LASTEXITCODE -ne 0) { throw "V21 Teacher $Mode failed with exit code $LASTEXITCODE" }
}
finally {
    Pop-Location
