param(
    ...
    [string]$OutputDir = 'teacher_runs/v19_safe_baseline/screen_v1',
...
try {
    ...
    if ($Seed -eq 0) { $Seed = if ($Mode -eq 'Screen') { 19100001 } else { 19200001 } }
    $arguments = @('-u','-m','teacher_rl.safe_continuous_rl')
    if ($Mode -eq 'Screen') {
    ...
    & $python @arguments
    if ($LASTEXITCODE -ne 0) { throw "V19 $Mode failed with exit code $LASTEXITCODE" }
}
